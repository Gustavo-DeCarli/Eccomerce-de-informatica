...
<body class="fundo">
    <header class="p-3 bg-dark text-white">
        <div class="container-fluid">
            <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
                ...
                <form action='perifericos.php' method='POST' class="col-12 col-lg-auto mb-3 mb-lg-0 me-lg-3">
                    <input type="search" name='busca' class="form-control form-control-dark text-light" placeholder="Search..." aria-label="Search">
                </form>
    ...
    <div class="container">
        <div class="row row-cols-1 row-cols-md-2 row-cols-lg-4 g-5">
            ...
            if (!isset($_POST['busca'])) {
                $dados = $connection->query("SELECT * from produtos WHERE id_categoria=2 ORDER BY RAND() LIMIT $inicio,$total_reg");
            } else {
                $nome = $_POST['busca'];
                $dados = $connection->query("SELECT * from produtos WHERE id_categoria=2 and nome LIKE '%$nome%' ORDER BY RAND() LIMIT $inicio,$total_reg");
            }
    ...
    <div class="mt-5 container justify-content-center"><?php
                                                        $tr = $dados->rowCount();
                                                        $tp = $tr / $total_reg;
                                                        $anterior = $pc - 1;
                                                        $proximo = $pc + 1;
                                                        if ($pc > 1) {
                                                            echo "<a class='col-md-4 btn btn-light' href='?pagina=$anterior'>Anterior</a>";
                                                        }
                                                        if ($pc < $tr) {
                                                            echo "<a class='btn btn-light col-md-4 offset-md-4' href='?pagina=$proximo'>Próxima</a>";
                                                        }
                                                        ?>
    </div>
